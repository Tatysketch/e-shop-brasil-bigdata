...
def get_database():
    ...
    client = MongoClient('localhost', 27017, username='admin', password='senha secreta123')
...
with st.form("adicionar_pedido_form"):
    ...
    if todos_clientes and todos_produtos:
        ...
        if submitted_pedido:
            if cliente_id_selecionado_str and produtos_ids_selecionados_str:
                ...
                pedido_data = {
                    ...
                    "data_pedido": datetime.now().strftime("%Y-%m-%d %H:%M:%S") # Data e hora atual
                }
                try:
                    pedidos_collection.insert_one(pedido_data)
                    st.success("Pedido adicionado com sucesso!")
                    # Opcional: st.experimental_rerun() para forçar a atualização da lista
                except Exception as e:
